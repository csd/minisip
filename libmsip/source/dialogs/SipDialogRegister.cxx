...
bool SipDialogRegister::a0_start_tryingnoauth_register( const SipSMCommand &command){
    ...
    if (transitionMatch(command, SipCommandString::proxy_register)){
			//SipDialogRegister *regcall = (SipDialogRegister *)sipStateMachine;
			...
			MRef<SipTransaction*> trans = new SipTransactionClient(/*regcall*/ this, seqNo, /*getDialogConfig().callId*/ callId);
			registerTransaction(trans);
			send_noauth(trans->getBranch());
                        CommandString cmdstr( callId, SipCommandString::register_sent);
...
bool SipDialogRegister::a2_tryingnoauth_tryingstored_401haspass( const SipSMCommand &command){
	...
	if ( hasPassword() && transitionMatch(command, SipResponse::type, IGN, SipSMCommand::TU, "401")){
                ...
                MRef<SipTransaction*> trans( new SipTransactionClient(this, seqNo, callId));
                registerTransaction(trans);
...
bool SipDialogRegister::a3_tryingnoauth_askpassword_401nopass( const SipSMCommand &command){
	...
	if ( hasPassword() && transitionMatch(command, SipResponse::type, IGN, SipSMCommand::TU, "401")){
		...
		//TODO: Ask password
                CommandString cmdstr( callId, SipCommandString::ask_password, getDialogConfig().inherited.sipIdentity->sipProxy.sipProxyIpAddr->getString());
		getDialogContainer()->getCallback()->sipcb_handleCommand( cmdstr );
	//extract authentication info from received response
		MRef<SipResponse*> resp( (SipResponse *)*command.getCommandPacket());
		setRealm( resp->getRealm() );
...
bool SipDialogRegister::a6_askpassword_registred_2xx( const SipSMCommand &command){
	...
	if (transitionMatch(command, SipResponse::type, SipSMCommand::transaction, IGN, "2**")){
		...
		//TODO: inform GUI
		if (getGuiFeedback()){
                        CommandString cmdstr( callId, SipCommandString::register_ok, getDialogConfig().inherited.sipIdentity->sipProxy.sipProxyIpAddr->getString());   
			getDialogContainer()->getCallback()->sipcb_handleCommand( cmdstr );
			setGuiFeedback(false);
		}
		requestTimeout(1000*60*14,SipCommandString::proxy_register);
...
SipDialogRegister::SipDialogRegister(MRef<SipDialogContainer*> dContainer, SipDialogConfig &callconf, MRef<TimeoutProvider<string, MRef<StateMachine<SipSMCommand,string>*> > *> tp)
...
{
	setUpStateMachine();
	callId = itoa(rand())+"@"+getDialogConfig().inherited.localIpString;
...
bool SipDialogRegister::handleCommand(const SipSMCommand &command){
	...
	if (command.getType()==SipSMCommand::COMMAND_STRING
		...
		if (callId == command.getCommandString().getDestinationId()){
...
//	mdbg << "SipDialogRegister: domain is "<< proxy_domain<< end;
	MRef<SipRegister*> reg= new SipRegister(branch, /*getDialogConfig().callId*/ callId,
